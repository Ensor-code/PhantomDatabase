    ...
    try:
        ...
        if name['icompstar'] == 2:
            if name['subst']== 11:  
                ...
                name["racc2"]= racc2b
            elif name['subst'] == 11: #primary mass is original primary mass, original secondary is divided into m2 and m3
...
def search_dir(loc, prefix, minDumpFiles=20):
    ...
    for path, _, files in os.walk(loc):
        ...
        if len(dumpFiles) >= minDumpFiles and len(setupFiles) >0 and len(inFiles)>0:
